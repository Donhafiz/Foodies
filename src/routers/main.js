...
route.get("/admin/addDish", (req, res) => {
  if (req.session.loginUser) {
    ...
    if (loginUser.type == "admin") {
      ...
      res.send(
        `<h1>Something Wrong !!</h1> <h2>Opps! you can not acess this page..</h2>`
      );
    }
  } else
    res.render("login", {
...
route.get("/admin/deleteDish/:id", async (req, res) => {
  if (req.session.loginUser) {
    ...
    if (data) {
      ...
      res.send(
        "<h1>Server Error !!</h1><h2> Soory dish is not deleted p;ease try letter..</br><a href='/admin/dishMenus/1'>Back to dish menu table</a> </h2>"
      );
    }
  } else
    res.render("login", {
...
route.get("/admin/cooking/:id", async (req, res) => {
  if (req.session.loginUser) {
    ...
    } else res.send("<h2>Wrong page try to access...</h2>");
  } else {
    res.render("login", {
      loginFirst: true,
    });
  ...
  if (req.session.loginUser) {
    ...
    if (loginUser.type == "admin") {
      ...
      if (data) {
        ...
        res.send(
          `<h1>Something Wrong !!</h1> <h2>Opps! Dish not Found...</h2> `
        );
      }
    } else {
      res.send(
...
route.post("/admin/saveEditDish/:id", async (req, res) => {
  if (req.session.loginUser) {
    if (req.files == null) console.log("photo not selected");
    else {
      console.log("photo to is selected ols photo is " + req.body.tempImage);
      try {
        fs.unlinkSync("./public/dishImage/" + req.body.tempImage);
    ...
    const data = await dish.updateOne(
      { _id: req.params.id },
      { $set: req.body }
    );
    if (data) {
...
route.post("/orderNowFromBasket", async (req, res) => {
    ...
    try {
      ...
      // Use map instead of forEach and wait for all promises to resolve
      const orderPromises = basket.map(async (item) => {
        let object = {
          dishId: item.id,
        ...
        // Calculate item total and add to totalPrice
        const itemTotal = item.price * item.quantity;
        return itemTotal;
      });
  
      // Wait for all orders to be created and sum up the total price
      const itemTotals = await Promise.all(orderPromises);
      totalPrice = itemTotals.reduce((sum, itemTotal) => sum + itemTotal, 0);
        ...
        line_items: [
          {
            price_data: {
              currency: "usd",
              product_data: {
                name: 'Purchase of all food in cart',
...
route.get("/create-payment-intent/:id", async (req, res) => {
  const { id } = req.params;
  try {
    const food = await dish.findById(id);
    if (!food) {
      return res.status(404).json({ message: "Unable to proceed to payment" });
    }
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              name: food.dname,
              description: food.description || "No description available", // Add a fallback description
              images: [food.photo],
            },
            unit_amount: parseInt(food.dprice * 100), // Stripe expects amount in cents
          },
          quantity: 1,
        },
      ],
